@include('layout.head')
<div class="min-h-svh bg-third">
    <div class="px-16 font-poppins text-center py-20 max-w-7xl">
        <div class="card-wrapper flex justify-center items-center gap-8">
            <div class="card bg-white px-28 py-12 rounded-xl shadow-lg flex gap-8">
                <img src="/img/examination.png" class="" alt="">
                <div class="content-center">
                    <a href="/beginner/exam"
                        class="text-white bg-primary inline-block hover:opacity-85 rounded-xl text-xs px-6 py-3 my-auto mx-auto ">START
                        EXAM</a>
                </div>
            </div>
        </div>
    </div>
</div>
@include('layout.footer')
